...
import connectionGenerator, { databaseCreator, dbTestOptions, initialValues, resetDatabase, runMigrations } from "../src/database/init-db";
...
beforeAll(async () => {
  ...
  //Run the migrations
  runMigrations(drizzle(connection), __dirname + "/../src/database/migrations");
