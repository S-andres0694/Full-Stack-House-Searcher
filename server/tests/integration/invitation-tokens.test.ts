...
import connectionGenerator, { dbTestOptions, initialValues, resetDatabase } from '../../database/init-db';
...
describe('Tests that when registering as a new user, an invitation token is created', () => {
	it('should create an invitation token', async () => {
        ...
        const invitationTokenRecord: InvitationToken[] = await db.select().from(invitationTokens).where(eq(invitationTokens.token, invitationToken));
    ...
    it('shows that consuming the token removes marks it as invalid', async () => {
        const invitationToken: string = "1234567890";
        const invitationTokenRecord: InvitationToken | undefined = await invitationTokenModel.createInvitationToken(invitationToken);
        expect(invitationTokenRecord).toBeDefined();
        expect(invitationTokenRecord).not.toBeUndefined();
        await invitationTokenModel.consumeInvitationToken(invitationTokenRecord as InvitationToken);
        expect(await invitationTokenModel.isInvitationTokenValid(invitationToken)).toBe(false);
    });
    ...
    it('shows a user is able to register with an invitation token', async () => {
        ...
        const registerResponse: Response = await fetch(`http://localhost:${port}/auth/register`, {
            method: 'POST',
            headers: {
        ...
        const userRecord: User | undefined = await usersModel.getUserByUsername(user.username);
    ...
    it('shows that a user cannot register with an invalid invitation token', async () => {
        ...
        const registerResponse2: Response = await fetch(`http://localhost:${port}/auth/register`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        expect(jsonResponse2.message).toBe('Invitation token has already been used.');
    });
