...
import { independentUser, user, user2, userSameEmailAsUser } from "../constants";
...
describe("User API Testing", () => {
  ...
  it("tests that the create user endpoint fails when the email already exists", async () => {
    await userModel.createUser(user);
    const response = await request(app).post("/users").send(userSameEmailAsUser);
    expect(response.status).toBe(409);
  ...
  it("tests that the getUserByEmail endpoint fails when the user does not exist", async () => {
    const response = await request(app).get("/users/email/nonexistent@test.com");
    expect(response.status).toBe(404);
    expect(response.body.error).toBe("User not found");
  });
  ...
  it("tests that the updateUserUsername endpoint works", async () => {
    await userModel.createUser(user);
    const userId: number = (await userModel.getUserId(user.username))!;
    const response = await request(app).put(`/users/${userId}/username`).send({ newUsername: "newusername" });
    expect(response.status).toBe(204);
  ...
  it("tests that the updateUserUsername endpoint fails when the new username already exists", async () => {
    await userModel.createUser(user);
    await userModel.createUser(independentUser);
    const userId: number = (await userModel.getUserId(independentUser.username))!;
    const response = await request(app).put(`/users/${userId}/username`).send({ newUsername: user.username });
    expect(response.status).toBe(409);
    expect(response.body.error).toBe("Username already exists");
    expect((await userModel.getUserById(userId))?.username).toBe(independentUser.username);
  });
  ...
  it("tests that the updateUserUsername endpoint fails when the new username is the same as the current username", async () => {
    ...
    const response = await request(app).put(`/users/${userId}/username`).send({ newUsername: user.username });
    expect(response.status).toBe(400);
    expect(response.body.error).toBe("Cannot update username to the same username");
  });

  it("tests that the updateUserUsername endpoint fails when the user does not exist", async () => {
    const response = await request(app).put("/users/-1020/username").send({ newUsername: "newusername" });
    expect(response.status).toBe(404);
    expect(response.body.error).toBe("User not found");
  });
  ...
  it("tests that the updateUserEmail endpoint works", async () => {
    await userModel.createUser(user);
    const userId: number = (await userModel.getUserId(user.username))!;
    const response = await request(app).put(`/users/${userId}/email`).send({ newEmail: "newemail@test.com" });
    expect(response.status).toBe(204);
    expect((await userModel.getUserById(userId))?.email).toBe("newemail@test.com");
  });
  ...
  it("tests that the updateUserEmail endpoint fails when the new email already exists", async () => {
    ...
    await userModel.createUser(independentUser);
    const userId: number = (await userModel.getUserId(independentUser.username))!;
    const response = await request(app).put(`/users/${userId}/email`).send({ newEmail: user.email });
    expect(response.status).toBe(409);
    expect(response.body.error).toBe("Email already exists");
    expect((await userModel.getUserById(userId))?.email).toBe(independentUser.email);
  });
  ...
  it("tests that the updateUserEmail endpoint fails when the user does not exist", async () => {
    const response = await request(app).put("/users/-1020/email").send({ newEmail: "newemail@test.com" });
    expect(response.status).toBe(404);
    expect(response.body.error).toBe("User not found");
  });
  ...
  it("tests that the hasRole endpoint works", async () => {
    await userModel.createUser(user);
    const userId: number = (await userModel.getUserId(user.username))!;
    const response = await request(app).get(`/users/${userId}/hasRole/${user.role}`);
    expect(response.status).toBe(200);
  ...
  it("tests that the hasRole endpoint fails when the role does not exist", async () => {
    await userModel.createUser(user);
    const userId: number = (await userModel.getUserId(user.username))!;
    const response = await request(app).get(`/users/${userId}/hasRole/nonexistentrole`);
    expect(response.status).toBe(404);
