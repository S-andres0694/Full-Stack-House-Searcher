...
export const Links: FunctionComponent<LinksProps> = ({
	...
	const hoverAnimationsHandler: MouseEventHandler<
		HTMLAnchorElement
	> = (): void => {
	...
	const hoverAnimationsLeaveHandler: MouseEventHandler<
		HTMLAnchorElement
	> = (): void => {
		linkStateAnimations.start({
			background: 'transparent',
	...
	return (
		<AnimatedLink
			style={linkState}
			className="p-2 px-4 rounded-full dark:hover:text-slate-800 hover:text-white hover:shadow-slate-800 dark:hover:shadow-white hover:shadow-2xl"
			to={to}
