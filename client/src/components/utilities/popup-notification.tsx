...
export const PopupNotification: FunctionComponent<PopupNotificationProps> = ({
    ...
    return (
        <Alert size={size} title={errorTitle} status={type}>
            {errorMessage}
        </Alert>
    );
};
...
export function notify(errorTitle: string, errorMessage: string, type: 'error' | 'success' | 'info' | 'warning' | 'neutral') {
    toast(<PopupNotification errorTitle={errorTitle} errorMessage={errorMessage} type={type} />, {
        closeButton: false,
        position: 'top-right',
        closeOnClick: true,
        pauseOnHover: true,
